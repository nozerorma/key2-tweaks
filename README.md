...
- **Audio FX** — global session-0 effects don't process media on this Qualcomm ROM. Like LineageOS AudioFX, the app attaches `Equalizer` / `BassBoost` / `LoudnessEnhancer` to **each media app's own audio session** (via the `OPEN/CLOSE_AUDIO_EFFECT_CONTROL_SESSION` broadcasts). The effects are hosted in the long-lived accessibility process, so no foreground-service notification is needed.
